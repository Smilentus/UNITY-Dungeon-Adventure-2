            ...
            if (uType != SkillType.None)
                switch (uType)
                {
                    ...
                    case SkillType.ExtraInvSlot:
                        FindObjectOfType<Inventory.Inventory>().OpenInvSlot(RuntimePlayer.Instance.RuntimePlayerStats.openedInvCases);
                        RuntimePlayer.Instance.RuntimePlayerStats.openedInvCases += (int)var;
                        break;
                    case SkillType.ExtraRuneSlot:
                        FindObjectOfType<Inventory.Inventory>().OpenRuneSlot(FindObjectOfType<Inventory.Inventory>().inventory.Length - 8 + RuntimePlayer.Instance.RuntimePlayerStats.openedRuneCases);
