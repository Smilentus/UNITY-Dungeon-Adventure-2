public class BattleActionExecuter_HeavyStrike : BattleActionExecuter_BaseActionExecuter
{
    public override void ExecuteAction()
    {
        ...
        if (!RuntimeBattlePlayerController.Instance.CriticalStrike())
        {
            ...
            for (int i = 0; i < RuntimePlayer.Instance.RuntimePlayerStats.AttackSpeed; i++)
            {
                ...
                {
                    if (UnityEngine.Random.Range(0, 101) <= RuntimePlayer.Instance.RuntimePlayerStats.HeavyStrikeChance + RuntimePlayer.Instance.RuntimePlayerStats.Luck)
                    {
